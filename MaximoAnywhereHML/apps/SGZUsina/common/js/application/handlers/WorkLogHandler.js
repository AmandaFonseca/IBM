...
/**@memberOf application.handlers.WorkLogHandler */
		...
		initNewWorkLogEntry: function(eventContext){
			...
			if(!view.isOverrideEditMode()){
				workLogSet = CommonHandler._getAdditionalResource(this,"serviceRequest.workloglist");
		...
		enableAddWorkLogButton : function(eventContext) {
			...
			if(!serviceRequest || serviceRequest.isErrored() || !WorkLogObject.canAddWorkLog(serviceRequest)) {
		...
		validateLogType: function(eventContext){
			var workLog = CommonHandler._getAdditionalResource(eventContext,'serviceRequest.workloglist').getCurrentRecord();
		...
		_saveTransaction: function(){
			try{
     			var workOrderSet = CommonHandler._getAdditionalResource(this,"serviceRequest");
