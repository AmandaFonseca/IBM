...
/**@memberOf application.handlers.CalibrationPointHandler */
		...
		performCalculation: function(eventContext) {
			...
			try {
				if(CalibrationPointObject.isAnalog()) {					
					...
					if(eventContext.resourceAttribute.indexOf('input') > 0){
						validate_result = this.validateInput(eventContext);
						if(validate_result && !validate_result.validate && eventContext.ui.dialogStack.length === 0){
							eventContext.textWidget.domNode.value = '';
							eventContext.textWidget.value = '';
							
							eventContext.application.showMessage(
								MessageService.createResolvedMessage(validate_result.messageKey, validate_result.params),
								function(eventContext){
									eventContext.getCurrentRecord().setNullValue(eventContext.resourceAttribute);
									eventContext.getCurrentRecord().setNullValue(eventContext.resourceAttribute.replace('_local'));
								}, eventContext);
		...
		_getMaxToleranceExceeded: function(point, prefix)  {
			...
			for(var i = 1; i < 5; i++) {
				...
				var error = _NumberFormatterMixin.fromDisplayableValue(point.get(prefix+'error'+i));
		...
		setCalPointsInputMode: function(eventContext) {
			...
			CalibrationPointObject.calculateInitialStatus(prefix, pointSet, assetfunc, datasheet);
			Logger.timerEnd('Status Calculation', 1);
			
			var idx = pointSet[prefix + '_curr_index']? pointSet[prefix + '_curr_index'] : 0;
			if(pointSet.data.length == 0)
                pointSet.setCurrentIndex(-1) ;
			else
