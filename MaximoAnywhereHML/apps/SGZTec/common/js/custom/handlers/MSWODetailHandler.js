...
define("custom/handlers/MSWODetailHandler", 
	   [ "dojo/_base/declare",
	     "dojo/_base/array",
		  "dojo/_base/lang",
...
/**@memberOf custom.handlers.MSWODetailHandler */
		...
		async_valLoccheckLocationFound: function(eventContext, currWO, location, locationSet) {	
			...
			var handler = eventContext.custom['custom.handlers.FailureCodeHandler'];
...
//-------------------------------------------------------				
		...
		initFollowUpWorkOrderView : function(eventContext) {
			...
			Logger.trace("[MSWODetailHandler] Initializing FollowUpWorkOrderView");
			...
			if (WL.StaticAppProps.APP_ID!="Inspection"){
				if (this.originalWorkOrder.get('classstructureid') != null) {
					Logger.trace("[MSWODetailHandler] Found a classification need to copy it to follow on workorder");
...
//					followUp.set('classificationdesc', originalWorkOrder.get('classificationdesc'));
					...
					this.originalWorkOrder.getModelDataSet("workOrderSpec", true).then(function(workOrderSpecs){
						Logger.trace("[MSWODetailHandler] Found existing specification attributes copying them to follow up");
					...
					Logger.trace("[MSWODetailHandler] No classification on the original workorder");
					eventContext.ui.show("WorkExecution.FollowUpWorkOrderView");
					eventContext.application.hideBusy();
				}				
			} else {
		...
		asyncYesOnWOLocationToAsset: function(eventContext){
			...
			var handler = eventContext.application['custom.handlers.FailureCodeHandler'];
