...
define("custom/handlers/WODetailHandlerSearch", 
	   [ "dojo/_base/declare",
	     "dojo/_base/array",
		  "dojo/_base/lang",
	     ...
	     "dojo/topic"],
function(declare, arrayUtil, lang, ApplicationHandlerBase, CommunicationManager, Workorder, SynonymDomain, ModelService, MessageService, CommonHandler, FieldUtil, PlatformRuntimeException, PlatformRuntimeWarning, UserManager, PlatformConstants, WpEditSettings, AsyncAwareMixin, Logger, FailureCodeHandler,PersistenceManager,GeoLocationTrackingService,MapProperties, topic) {
	var listSizeArray = ['tasklistsize', 'assignmentlistsize', 'materiallistsize', 'toollistsize', 'actuallaborlistsize', 'actualmateriallistsize', 'actualtoollistsize', 'workloglistsize', 'sketchlistsize', 'multiassetloclistsize', 'attachmentssize'];
	var attributes =    ["tasklist", "assignmentlist", "materiallist", "toollist", "actuallaborlist", "actualmateriallist", "actualtoollist", "workloglist", "sketchlist", "multiassetloclist", "attachments"];
...
/**@memberOf custom.handlers.WODetailHandlerSearch */
		...
		async_valLoccheckLocationFound: function(eventContext, currWO, location, locationSet) {	
			...
			//If asset exists, set the description for it
			currWO.set('location',location);
			currWO.set('locationdesc',validLocation?validLocation.get('description'):null);
			//Set only for presentation
			currWO.set('localLocationLd', validLocation?validLocation.get('locationld'):null);
			//Set calibration loop based on Location
			currWO.set('pluscloop', validLocation?validLocation.get('pluscloop'):null);
			...
			if(asset){			
				...
				var locfailurecode = validLocation?validLocation.get('failurecode'):null;
				if (locfailurecode && locfailurecode!=wofailurecode){
					this.setFailureCode(currWO,locfailurecode,validLocation?validLocation.get('failurecodedesc'):null);
					this.handleFailureCodePromise(handler.populateCurrentFCLevel(eventContext), eventContext);		
				}
			}			
		},
...
//-------------------------------------------------------				
				...
				ModelService.save(workOrderSet).then(function() {
					...
					if (isFollowUp){
						...
						//Switch to created for new workorders
							if(eventContext.ui.mainSplitter){
								eventContext.ui.hideCurrentView();
								eventContext.ui.getViewFromId('WorkExecution.WorkItemsView').setQueryBaseIndexByQuery(PlatformConstants.CREATED_QUERYBASE)
		...
		initFollowUpWorkOrderView : function(eventContext) {
			...
			if(eventContext.ui.combinedViews){
				eventContext.ui.getViewFromId("WorkExecution.FollowUpWorkOrderView").blockUser = true;
			}

			Logger.trace("[WODetailHandlerSearch] Initializing FollowUpWorkOrderView");
			...
			this.originalWorkOrder.getModelDataSet('multiassetloclist', true).then(function(multiAssetLocSet){
				followUp.getModelDataSet('multiassetloclist', true).then(function(followUpMultiAssetLocSet) {
					for(var i=0; i<multiAssetLocSet.count();i++){
						...
						newRec.set('assetnum', currRec.get('assetnum'));
			...
			if (WL.StaticAppProps.APP_ID!="Inspection"){
				if (this.originalWorkOrder.get('classstructureid') != null) {
					Logger.trace("[WODetailHandlerSearch] Found a classification need to copy it to follow on workorder");
...
//					followUp.set('classificationdesc', originalWorkOrder.get('classificationdesc'));
					...
					this.originalWorkOrder.getModelDataSet("workOrderSpec", true).then(function(workOrderSpecs){
						Logger.trace("[WODetailHandlerSearch] Found existing specification attributes copying them to follow up");
					...
					Logger.trace("[WODetailHandlerSearch] No classification on the original workorder");
					eventContext.ui.show("WorkExecution.FollowUpWorkOrderView");
					eventContext.application.hideBusy();
				}				
			} else {
...
//			var assetLoc = CommonHandler._getAdditionalResource(eventContext,'additionalasset');
		...
		async_va_CheckAssetFound : function (curWO, assetSet, asset, location, siteid, eventContext) {
			...
			if (assetSet.count() == 0) {
				...
				//If asset exists, set the description for it
				newAsset = assetSet.getRecordAt(0);
		...
		handleFailureCodePromise: function(promise, eventContext) {
			var view = eventContext.ui.getCurrentViewControl();
			promise.then(function() {
				if (!view.id.includes("Lookup"))
					view.refresh();
			})
			.otherwise(function(exception) {
				if(typeof exception == 'PlatformRuntimeWarning') {
		...
		setSearchQuery: function(eventContext){
			...
			let wonum = search.wonum;
			var workOrderSet = CommonHandler._getAdditionalResource(this,"workOrder");
			CommonHandler._clearFilterForResource(eventContext,workOrderSet);
			let currItem = workOrderSet.filter("wonum == $1", wonum);

			let parentwonum; 
			workOrderSet.data.forEach(element => {
				console.log(element.wonum)
				if (element.wonum == wonum) {
					parentwonum = element.parentwonum
				}
			});

			search.wonum = parentwonum;
						...
						if (currentWoSet.getPersistentFilter()){
						     ...
						     then(function(){
									...
									ModelService.filtered('workOrder', PlatformConstants.SEARCH_RESULT_QUERYBASE, filter, null, null, null, oslcQueryParameters).then(function(resourceSet){
										...
										if (eventContext.ui.combinedViews) {
											...
											eventContext.ui.hideCurrentView();
											eventContext.ui.removeOverlay(eventContext.ui.getViewFromId(eventContext.ui.splitPane.rightPaneRootView).baseWidget);
											eventContext.application.showBusy();
											//eventContext.ui.getViewFromId(eventContext.ui.splitPane.rightPaneRootView).refresh();
											var df = eventContext.ui.getCombinedView().listView.lists[0].listWidget.getChildren()[0];
											eventContext.ui.getCombinedView().listView.lists[0].listWidget.setCurrentRecord(df);
											eventContext.application.hideBusy();
										}
									});
						     });
							...
							ModelService.filtered('workOrder', PlatformConstants.SEARCH_RESULT_QUERYBASE, filter, null, null, null, oslcQueryParameters).then(function(resourceSet){
								...
								if (eventContext.ui.combinedViews) {
									...
									eventContext.ui.movingBack = false;
									eventContext.ui.getViewFromId(eventContext.ui.splitPane.rightPaneRootView).refresh();
									//var df = eventContext.ui.getCombinedView().listView.lists[0].listWidget.getChildren()[0];
									//eventContext.ui.getCombinedView().listView.lists[0].listWidget.setCurrentRecord(df);
									eventContext.ui.addOverlay(eventContext.ui.getViewFromId(eventContext.ui.splitPane.leftPaneRootView).baseWidget);
									eventContext.application.hideBusy();
									
									//we subscribe to this topic, because we need a way to whether the ui is display after loading a;ll model data. NO other way of knowing
									//This is to prevent user from clicking on other workorders while the current one is being displayed. 
									var listRefreshTopic = topic.subscribe('listSizeRefresh', function(eventContext){
						                    
										eventContext.ui.removeOverlay(eventContext.ui.getViewFromId(eventContext.ui.splitPane.leftPaneRootView).baseWidget);
										listRefreshTopic.remove();
		...
		refreshAllListSizes: function(eventContext){
			topic.publish('listSizeRefresh', eventContext);
