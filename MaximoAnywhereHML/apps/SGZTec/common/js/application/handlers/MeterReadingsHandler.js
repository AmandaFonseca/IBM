...
/**@memberOf application.handlers.MeterReadingsHandler */
		...
		assetMeterPromise: function(assetnum, metername, siteid) {
			...
			var meterFilter = {'assetnum' : assetnum, 'siteid' : siteid, 'metername' : metername};
			ModelService.filtered('assetMeters', null, meterFilter, 1000, false, false, null, true).then(function(assetMeterSet){
				assetMeterSet.filter("active == true && assetnum == $1", assetnum); //Filter set instead of using exactmatch index so users don't have to reinstall the app for added exact match index.
				deferred.resolve(assetMeterSet);
			}).otherwise(function(error){
				Logger.errorJSON('[MeterReadingHandler] assetMeterPromise error fetching meter for:',meterFilter);
				Logger.errorJSON('[MeterReadingHandler] assetMeterPromise error', error);
				deferred.reject();
			});
		...
		locationMeterPromise: function(location, metername, siteid) {
			...
			var meterFilter = {'location' : location, 'siteid' : siteid, 'metername' : metername};
			ModelService.filtered('locationMeters', null, meterFilter, 1000, false, true, null, true).then(function(locationMetersSet){
				locationMetersSet.filter("active == true && location == $1", location); //Filter set instead of using exactmatch index so users don't have to reinstall the app for added exact match index.
				deferred.resolve(locationMetersSet);
			}).otherwise(function(error){
				Logger.errorJSON('[MeterReadingHandler] locationMeterPromise error fetching meter for:',meterFilter);
				Logger.errorJSON('[MeterReadingHandler] locationMeterPromise error', error);
				deferred.reject();
			});
			...
			ModelService.save(woSet).then(function() {
				...
				assetMeterPromise.then(function(assetMeterSet){
					if ((assetMeterSet.count()>0)) {	
						...
						if ((meterType == 'CONTINUOUS') && (myLocalMeter.getPendingOrOriginalValue('isdelta') == true) && !(isNaN(numberUtil.parse(myLocalMeter['lastreading'])))){
						...
						Logger.error('[MeterReadingHandler] commitEntry. No asset meter returned from the JSONStore so it cannott be saved.');
						eventContext.application.hideBusy();
						var message = MessageService.createResolvedMessage('saveFailed');
		...
		commitLocationEntry: function(eventContext) {
			...
			ModelService.save(woSet).then(function() {
				self.locationMeterPromise(location, metername, siteid).then(function(locationMeterSet){
					if ((locationMeterSet.count()>0)) {	
						...
						Logger.error('[MeterReadingHandler] commitLocationEntry. No asset meter returned from the JSONStore so it cannott be saved.');
						eventContext.application.hideBusy();
						var message = MessageService.createResolvedMessage('saveFailed');
						self.application.showMessage(message);
					}
