...
define("custom/handlers/MStatusChangeHandler", 
	   [ "dojo/_base/declare",
	     "dojo/_base/array",
...
/**@memberOf custom.handlers.MStatusChangeHandler */
		...
		commitAssetStatusChange: function(eventContext){			
			...
			if (asset.get("classstructureid") == '02.01') {
				if (newStatus == 'ATIVO') {
					...
					var currAssetSet = assetSet.getCurrentRecord();
					var self = this;
					var attachments_size = currAssetSet.get("attachmentssize");
					let attachments0;
					let attachmentsToday = 0;
		
		
					if (eventContext.getResource().getCurrentRecord().get('attachments')) {
						attachments0 = eventContext.getResource().getCurrentRecord().get('attachments');
						let date =  new Date();
						...
						attachments0.data.forEach(element => {
					...
					if (attachments0 == 0 || attachments0 < 2) {
					...
					var currAsset = CommonHandler._getAdditionalResource(eventContext,'asset.MSArbo').getCurrentRecord();
					...
					var attachmentsSet = eventContext.getResource().getCurrentRecord().get('attachments');
					...
					if(attachmentsSet != "--" || attachmentsSet != null || attachmentsSet != undefined){
						if (attachmentsSet.count() < 2){
							let msg = `O ativo deve possuir ao menos ${minimumAttachmentsSize} foto(s) para alterar o status para ATIVO.`;
						throw new PlatformRuntimeException(msg);
						}
					}
