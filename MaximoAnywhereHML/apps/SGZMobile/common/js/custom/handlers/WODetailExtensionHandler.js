				...
				[ ApplicationHandlerBase, AsyncAwareMixin ],
				{
					...
					readOnlySpec : function(eventContext) {
						...
						if (currentWorkOrder.get('ms_emergency')) {
							emergency = currentWorkOrder.get('ms_emergency');
						}
						...
						if (currentWorkOrder != null && currentWorkOrder.workOrderSpec != null && Spec !== undefined){
							for (var i = 0; i < Spec.count(); i++) {
								...
								if (currSpec == 'AREA' || currSpec == 'CALCULO_ESPESSU' || currSpec == 'VOLUME' ){
									if (currWorkOrderSpec.get('numvalue') == null || currWorkOrderSpec.get('numvalue') == ""){
										currWorkOrderSpec.set("numvalue",parseFloat("0,0"));
									}
									currWorkOrderSpec.getRuntimeFieldMetadata("numvalue").set("readonly");
									currWorkOrderSpec.getRuntimeFieldMetadata("alnvalue").set("readonly");	
								}
