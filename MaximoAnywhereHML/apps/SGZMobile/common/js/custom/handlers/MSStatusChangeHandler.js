  ...
  return declare([ApplicationHandlerBase, StatusChangeHandler], {
    ...
    filterWOStatus: function (eventContext) {
      ...
      /* Remover rotina deixar somente para aprovado.
					...
					} */ if (woStatusDomain.isFiltered() && currStatus == "APROVADO") {
        woStatusDomain.filter("value!=$1", "CONC");
      }
    },
    ...
    ticketLookup: function (eventContext) {
      ...
      var amtickets = CommonHandler._getAdditionalResource(
        eventContext,
        "amtickets"
      );
    ...
    hide_WODetailsView: function (eventContext) {
      ...
      var workOrderSet = CommonHandler._getAdditionalResource(
        eventContext,
        "workOrder"
      );
    ...
    hide_timesheetFields: function (eventContext) {
      ...
      //Logger.error("WORKTYPE: " + currWO.get("worktype"));
      if (
        currWO.get("worktype") != "TS" ||
        currWO.get("worktype") == null ||
        currWO.get("status") == "CONC"
      ) {
        eventContext.setDisplay(false);
        eventContext.setVisibility(false);
      }
    ...
    save_ini: function (eventContext) {
      ...
      var attachments = CommonHandler._getAdditionalResource(
        this,
        "attachments"
      );
    ...
    resolveStatusDate: function (statusdate) {
      ...
      return [
        FormatterService.toDisplayableValue(statusdate, "datetime", userLocale),
      ];
    },
    ...
    validateAttachments: function (eventContext) {
      var AttachmentSet = CommonHandler._getAdditionalResource(
        eventContext,
        "workOrder.attachments"
      );
      CommonHandler._clearFilterForResource(eventContext, AttachmentSet);
      var sections = CommonHandler._getAdditionalResource(
        eventContext,
        "photosessionlineResource"
      );
      CommonHandler._clearFilterForResource(eventContext, sections);
      var msDoclinksSet = CommonHandler._getAdditionalResource(
        eventContext,
        "msDoclinksResource"
      );
      ...
      for (var i = 0; i < reqSections.count(); i++) {
        ...
        var SectionDoclinks = msDoclinksSet.find(
          "ms_photosessionid == $1",
          photosessionid
        );
        ...
        if (SectionDoclinksCount < ms_minqty) {
          ...
          var strMsg =
            ...
            "precisa de ao menos " +
            ms_minqty +
            "fotos para alterar o status da OS.";
    ...
    commitWOStatusChange: function (eventContext, skipDynamicCheck) {
      ...
      var currentWorkOrder = eventContext.application
        .getResource("workOrder")
        .getCurrentRecord();
      var psconfigid = currentWorkOrder.get("ms_psconfigid");
      let ms_invalidmsgnaorealizadaCod =
        "Preencha o campo Código Motivo com a justificativa da não realização do serviço.";

      //Validate if status date change is lesser than last WO status change date
      if (
        currentWorkOrder.getAsDateOrNull("changestatusdate") >
        statusChange.getAsDateOrNull("changedate")
      ) {
        throw new PlatformRuntimeException("statusDateinFuture", [
          this.resolveStatusDate(
            currentWorkOrder.getAsDateOrNull("changestatusdate")
          ),
        ]);
      }
          ...
          for (var i = 0; i < reqSections.length; i++) {
            ...
            if (SectionDoclinksCount < minqty) {
              throw new PlatformRuntimeException(
                "ms_invalidstatusattachmentqtdcat",
                [sectionDesc, minqty]
              );
            }
          }
      ...
      if (
        ...
        currentWorkOrder.workOrderSpec != null &&
        statusChange.get("status") == "CONC"
      ) {
        ...
        var spec = CommonHandler._getAdditionalResource(
          eventContext,
            ...
            if (
              ...
              eventContext.ui.showMessage(
                MessageService.createStaticMessage(
                  "O campo 'Cura de concreto' precisa ser preenchido!"
                ).getMessage()
              );
            } else {
              ...
              try {
                isBoolean =
                  newDateFomatada instanceof Date && !isNaN(newDateFomatada);
                if (
                  isBoolean == undefined ||
                  isBoolean == null ||
                  isBoolean == false
                  ...
                  isBoolean =
                    newDateFomatada instanceof Date && newDateFomatada != null;
                }
              } catch (error) {
                console.log(error);
              }
              ...
              if (isBoolean) {
                if (newDateFomatada < nowDate) {
                  throw new PlatformRuntimeWarning(
                    "O campo 'Cura de concreto' precisa ser maior que a data atual!"
                  );
      ...
      if (newStatus == "EMAND") {
        var fieldMetadataPlate =
          workOrder.getRuntimeFieldMetadata("ms_woplate");
    ...
    blockFields: function (eventContext) {
      ...
      if (woStatus == "CONC") {
        ...
        var starttimeMetadata =
          statusChange.getRuntimeFieldMetadata("starttime");
        var finishtimeMetadata =
          statusChange.getRuntimeFieldMetadata("finishtime");
    ...
    hideFooterLookup: function (eventContext) {
      ...
      document
        .getElementById("sgzMobile.amcrewLookup_footer")
    ...
    hideReason: function (eventContext) {
      ...
      let workOrder = CommonHandler._getAdditionalResource(
        eventContext,
        "workOrder"
      );
    ...
    openPDWhy: function (eventContext) {
      ...
      var selectedRecord = CommonHandler._getAdditionalResource(
        eventContext,
        "whylist"
      ).getCurrentRecord();
