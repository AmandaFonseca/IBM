  ...
  function(declare, all, ModelService, ModelData, ModelDataSet, ApplicationHandlerBase, CommunicationManager, WorkOrderTimer, WorkOrderObject, UserManager, MessageService, lang, PlatformRuntimeException, PlatformRuntimeWarning, PlatformConstants, Logger, MaxVars, CommonHandler,WOGeolocationHandler) {
	return declare(ApplicationHandlerBase, {
		...
		render : function (eventContext) {
			...
			//Cleanup all the sketches - If the SPATIAL object exists
		...
		handleUnsavedSketches: function(eventContext) {
			//Check for unsaved sketches - If the SPATIAL object exists
			var workOrderSet = eventContext.application.getResource("workOrder");
			this.mobileMaximoSpatial = workOrderSet.mobileMaximoSpatial;
			if (this.mobileMaximoSpatial != null && this.mobileMaximoSpatial.sketchTool != null) {
				
				var sketchTool = this.mobileMaximoSpatial.sketchTool;
				if (sketchTool.isCurrentSketchSaved == false) {
					eventContext.application.ui.show('WorkExecution.UnsavedSketch');
				} else {
					if (eventContext.ui.combinedViews) {
						sketchTool.clearSketches();	
					}
					if (sketchTool.isOpened) {
						sketchTool.hideDialog();
					}
					this.mobileMaximoSpatial.setZoomStatus(null);
...
/**@memberOf application.handlers.WOListHandler */
		...
		enableTimerButton : function(eventContext) {	
			...
			if(workOrderTimerSet){
				...
				} else { //Force the current woTimer.wonum to use the started icon
					if (workOrder.get("wonum") && workOrder.get("started") && (woTimer.get("wonum") == workOrder.get("wonum"))) {
						eventContext.setState(true);
					}
				} 
