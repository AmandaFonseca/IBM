...
*/
		...
		populateSearchAdress: function(eventContext, search){
			...
			if (search.ms_addressline.includes('RUA')){
				let split = search.ms_addressline.split("RUA")[1];
				split = split.trim();
				search.ms_addressline = `R  ${split}`

			} 
			
			if(search.ms_addressline.includes('R ')){
				let split = search.ms_addressline.split("R ")[1];
				split = split.trim();
				search.ms_addressline = `R  ${split}`			
			}
			...
			if (search.ms_addressline.includes('AVENIDA')){
				let split = search.ms_addressline.split("AVENIDA")[1];
				split = split.trim();
				search.ms_addressline = `AV  ${split}`

			} 
			
			if(search.ms_addressline.includes('AV ')){
				let split = search.ms_addressline.split('AV ')[1];
				split = split.trim();
				search.ms_addressline = `AV  ${split}`
			}
			...
			if (search.ms_addressline && !search.ms_staddrnumber) {
				...
				lista.filter('ms_addressline == $1', search.ms_addressline);				
			}

			if (search.ms_addressline && search.ms_staddrnumber) {
				CommonHandler._clearFilterForResource(eventContext, lista);
				lista.filter('ms_addressline == $1  && ms_staddrnumber ==$2', search.ms_addressline , search.ms_staddrnumber);		
			}
...
/*--- limpar campos de especificação da arvore ------------- */
		...
		hideTechReportContainer : function(eventContext){
			...
			var deadtree = currAsset.get("ms_deadtree");
			var validate = currAsset.get("ms_validate");
			let crea = currAsset.get('crea')
			let crbio = currAsset.get('crbio')
		...
		setDeadTree : function(eventContext){
			...
			if (attachments == null || attachments < 0 || attachments.includes('--')) {
